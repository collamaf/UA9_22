...
class AnaMuEvent {
	...
	AnaMuEvent(TTree *tree=0, TString theRunName="run");
	virtual ~AnaMuEvent();
...
#ifdef AnaMuEvent_cxx
AnaMuEvent::AnaMuEvent(TTree *tree, TString theRunName) : fChain(0)
{
...
AnaMuEvent::~AnaMuEvent()
{
	if (!fChain) return;
	delete fChain->GetCurrentFile();
}

Int_t AnaMuEvent::GetEntry(Long64_t entry)
{
	// Read contents of entry.
	if (!fChain) return 0;
	return fChain->GetEntry(entry);
}
Long64_t AnaMuEvent::LoadTree(Long64_t entry)
{
...
void AnaMuEvent::Init(TTree *tree)
{
...
void AnaMuEvent::Show(Long64_t entry)
{
...
Int_t AnaMuEvent::Cut(Long64_t entry)
{
